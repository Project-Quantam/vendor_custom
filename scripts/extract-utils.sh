...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$ROOT"/vendor/custom/scripts/smali/baksmali.jar
        export SMALIJAR="$ROOT"/vendor/custom/scripts/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$ROOT"/vendor/custom/scripts/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$ROOT"/vendor/custom/scripts/compact_dex_converter
    fi
